...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    // Replace the following line with your solution.
    	...
    	else if (key.compareTo(node.entry.key) > 0){//少一个else的bug！
    		if (node.rightChild == null){
    			return null;
    		}
    		else{
    	...
    	if (node.equals(root)){
    		if(node.leftChild != null && node.rightChild != null){
    			...
    			if(X.rightChild != null){
    				...
    				root.leftChild.parent = X;
    				X.rightChild = root.rightChild;
    				root.rightChild.parent = X;
    				root = X;
    			}
    			else{
    		...
    		if(node.leftChild != null && node.rightChild != null){
    			...
    			if(X.rightChild != null){
    				...
    				if(((Comparable)X.entry.key).compareTo(node.parent.entry.key) > 0){
...
//    System.out.println(tree.root.rightChild.leftChild.rightChild);
