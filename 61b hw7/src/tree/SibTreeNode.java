...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
	  if(myTree==null){//判定myTree
		  throw new InvalidNodeException();
	  }
	  SibTreeNode node = new SibTreeNode(myTree,item);
	  node.parent = this;
		  
	  if(c<=1||this.children()==0){
		  node.nextSibling = this.firstChild;
		  this.firstChild = node;
	  }
	  else if(c>this.children()){
		  ((SibTreeNode)child(this.children())).nextSibling = node;
	  }
	  else{
		  node.nextSibling = ((SibTreeNode)child(c-1)).nextSibling;
		  ((SibTreeNode)child(c-1)).nextSibling = node;
	  }
	  myTree.size++;//注意myTree的size
  }
		  ...
		  if(this.children()==0){
			 if(this==myTree.root){
				 myTree.root=null;
			  	...
			  	SibTreeNode curr = this.parent.firstChild;
			  	if(curr==this){
